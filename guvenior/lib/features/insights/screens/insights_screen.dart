...
class _InsightsScreenState extends State<InsightsScreen>
  ...
  Future<void> _loadData() async {
    try {
      final response = await InsightService.generateInsights();
  ...
  double get _nightSpendingRatio {
    if (_analysis != null) return _analysis!.nightSpendingRate;
  ...
  Widget _buildInsightsSection() {
    ...
    return Column(
      ...
      children: [
        ...
        ..._insights.map((insight) => _buildInsightTile(insight)),
      ],
    );
  }
  ...
  Widget _buildInsightTile(Insight insight) {
    ...
    Color themeColor = const Color(0xFF00E5A0);
    
    final titleLower = insight.title.toLowerCase();
    if (titleLower.contains('maas') || titleLower.contains('maaş')) {
      iconData = Icons.work_outline_rounded;
      themeColor = AppColors.peach;
    } else if (titleLower.contains('gece')) {
      iconData = Icons.nights_stay_rounded;
      themeColor = const Color(0xFFA29BFE);
    } else if (titleLower.contains('butce') || titleLower.contains('bütçe')) {
      iconData = Icons.warning_amber_rounded;
      themeColor = AppColors.sky;
    } else if (titleLower.contains('harcama')) {
      iconData = Icons.shopping_bag_outlined;
      themeColor = AppColors.moodStressed;
    } else if (titleLower.contains('birikim')) {
      iconData = Icons.savings_outlined;
      themeColor = const Color(0xFF00E5A0);
    }

    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: GlassCard(
        borderColor: themeColor.withOpacity(insight.isRead ? 0.1 : 0.4),
        child: Row(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Container(
