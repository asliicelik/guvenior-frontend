...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      setState(() {
        ...
        _salary = results[2] as double;
        _streak = results[4] as int;
        _monthlyReport = results[5] as MonthlyReport;
        _isLoading = false;
      });
      _cardController.forward();
    } catch (_) {
      // If monthly report fails (e.g. initial run/auth issue), fallback gracefully
      try {
        final results = await Future.wait([
          IncomeService.getIncomes(),
          ExpenseService.getExpenses(),
  ...
  double get _netBalance => _salary + _totalIncome - _totalExpense;
  double get _spendingRatio =>
      (_salary + _totalIncome) > 0
          ? (_totalExpense / (_salary + _totalIncome)).clamp(0.0, 1.0)
          : 0.0;
  ...
  String get _aiMessage {
    if (_monthlyReport != null && _monthlyReport!.aiSummary.isNotEmpty) {
      return _monthlyReport!.aiSummary;
    }
    if (_monthlyReport != null && _monthlyReport!.ruleBasedSummary.isNotEmpty) {
      return _monthlyReport!.ruleBasedSummary;
    }
    if (_salary == 0 && _totalIncome == 0) {
      return 'Henüz gelir eklemedin. Ayarlardan maaşını gir! 🚀';
    }
  ...
  Color get _aiColor {
    final profile = _monthlyReport?.analysis.behaviorProfile;
    if (profile == 'Hedef Odaklı' || profile == 'Dengeli İlerleyen') {
      return const Color(0xFF00E5A0);
    }
    if (profile == 'Maaş Sonrası Hızlanan' || profile == 'Gece Harcamasına Açık') {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _balanceChip('Maaş', _salary, AppColors.peach, Icons.work_outline),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.auto_awesome, color: _aiColor, size: 18),
              const SizedBox(width: 8),
              Text(
                'AI Koçun diyor ki:',
                style: GoogleFonts.inter(
              ...
              if (profile != null && profile.isNotEmpty) ...[
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(
                    color: _aiColor.withOpacity(0.2),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: _aiColor.withOpacity(0.3)),
                  ),
                  child: Text(
